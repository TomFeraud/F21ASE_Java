...
public class PassengerQueue {
	...
	/**
	 * Consumer method Poll a passenger at the head of the Queue Return passenger
	 * information and flight information for observer to update
	 *
	 * @param deskNo
	 *            Desk Number
	...
	/**
	 * Produce the queue add all passengers to the queue randomly then set empty to
	 * be false and notify consumer to start
	 */
	...
	/**
	 * Add the passenger to the corresponding flight and get the current flight
	 * information
	 *
	 * @param passenger
	 *            Passenger
	...
	public synchronized String getQueuePassengers() {
		...
		for (Passenger p : queue) {
			queueText.append(bookingList.getPassengerBookingRef(p.getFullName())).append("        ")
					.append(p.toString()).append(p.getBaggage()).append("\n\n");
		}
